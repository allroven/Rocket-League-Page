    ...
    const animate = (timestamp) => {
      ...
      if (progress < t) {
        ...
        if (Math.abs(currentPos.y) >= FIELD_LIMIT) {
          if (Math.abs(currentPos.x) <= 60 / SCALE) {
            setGoalAlert(true);
            addLog(`¡GOL DETECTADO! Posición Y: ${currentPos.y.toFixed(2)}`, 'header');
            setIsAnimating(false);
            return;
          } else {
            addLog(`Impacto en Pared Trasera Y: ${currentPos.y.toFixed(2)} (Fuera del marco)`, 'warning');
            setIsAnimating(false);
            return;
          }
        }
    ...
    animationRef.current = setInterval(() => {
      ...
      if (Math.abs(currentPos.y) >= FIELD_LIMIT) {
        if (Math.abs(currentPos.x) <= 60 / SCALE) {
          ...
          const normal = new Vector3D(0, currentPos.y > 0 ? -1 : 1, 0);
          const r = v.reflect(normal);
  
          addLog(`Colisión en Pared Y: ${currentPos.y.toFixed(2)}`, 'warning');
          addLog(`Normal N = ${normal.toString()}`, 'data');
          addLog(`Reflejo R = ${r.toString()}`, 'success');
          ...
          v = r;
          // Ajustar posición para no quedarse pegado a la pared
          currentPos.y = currentPos.y > 0 ? FIELD_LIMIT - 1 : -FIELD_LIMIT + 1;
        }
      }
      ...
      <div className="w-full lg:w-1/3 space-y-6">
        ...
        <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 font-mono text-sm h-72 overflow-y-auto shadow-inner relative">
          ...
          <div className="space-y-1 mt-2">
            ...
            {logs.map((log, i) => (
              <div key={i} className={`flex items-start gap-2 ${log.type === 'header' ? 'text-yellow-400 font-bold mt-4 mb-2' :
